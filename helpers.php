...
{
    $sql = "SELECT COUNT(`project_id`) AS `task_count`, `project`.`id` AS `id`, `project`.`name` AS `name` FROM `task`
 ...
 * @param mysqli $conn Строка подключения
 * @param int $user_id id пользователя
 * @param int $project_id Название проекта, если передать пустое значение фильтрация списка по проекту не будет выполнена
 ...
 * @return false|mysqli_result Список задач или false в случае ошибки
 */
function db_get_tasks(mysqli $conn, int $user_id, int $project_id = 0, ?string $search_query = null, ?string $filter = null): false|mysqli_result
{
    ...
    } elseif ($filter === FILTER_TOMORROW) {
        $sql .= 'AND DATE(end_date) = DATE(NOW() + INTERVAL 1 DAY) ';
    } elseif ($filter === FILTER_OVERDUE) {
        $sql .= 'AND DATE(end_date) < DATE(NOW()) ';
    }
 ...
 */
function db_insert_task(mysqli $conn, string $name, int $user_id, int $project_id, ?string $date = null, ?string $file_path = null)
{
 ...
 * @return int 1 - если пользователь найден и 0 - если нет
 */
function db_project_exist_from_id(mysqli $conn, int $user_id, int $id): int
{
    $sql = "SELECT id FROM project WHERE user_id = $user_id AND id=$id";
    return $conn->query($sql)->num_rows;
}

function db_project_exist_from_name(mysqli $conn, int $user_id, string $name): int
{
    $sql = 'SELECT id FROM project WHERE user_id=? AND name=?';
    return send_query($conn, $sql, $user_id, $name)->num_rows;
}
 ...
 */
function db_update_task_status(mysqli $conn, int $user_id, int $task_id, int $status)
{
    if ($status > 1) {
        $status = 1;
    } elseif ($status < 0) {
    ...
    $sql = "UPDATE task SET is_completed = $status WHERE user_id = $user_id AND id = $task_id";
    $conn->query($sql);
}
 ...
 * @return false|mysqli_result результат запроса или false в случае его отсутсвия или позникновения ошибки
 */
 ...
 * Премещает файл
 *
 * @param array|null $file Ассоциативный массив представляющий файл
 * @param string $dir Путь к дериктории в которую нужно переместить файл
 * @return bool возвращает true при удачном перемещении и false при неудачном
 */
...
{
    $dateTimeObj = date_create_from_format(DATE_FORMAT, $date);
